...
namespace FocuSpider
{
    public class Focuser
    {
        ...
        public async Task StepAsync(bool isForward, int steps)
        {
            if (!this.IsConnected) return;

            await Task.Run(() =>
            {
                this.Step(isForward, steps);
            });
        }

        public void Step(bool isForward, int steps)
        {
            if (!this.IsConnected) return;
            ...
            {
                this.IsBusy = true;
                this._port.ReadTimeout = steps * 100;
